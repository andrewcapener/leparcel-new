...
export default async function ShowSettings() {
  ...
  return (
    <div style={{ padding: '26px 26px 80px', maxWidth: 900 }}>
      <h1 style={{ fontFamily: 'var(--font-c)', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '.012em', fontSize: 'var(--t-d2)', marginBottom: 8 }}>
      ...
      <div style={{ display: 'grid', gap: 0, borderTop: '2px solid var(--ink)' }}>
        {spaces.map((s) => (
          <form action={updateSpace} key={s.id}
            ...
            <span style={{ fontFamily: 'var(--font-c)', fontWeight: 600, fontSize: 'var(--t-lbl)', letterSpacing: '.05em' }}>{s.code}</span>
            <span style={{ textTransform: 'capitalize', fontSize: 'var(--t-lbl)', color: 'var(--ink-2)' }}>{s.track}</span>
            <input className="inp" name="label" defaultValue={s.label} aria-label="Label" style={{ padding: '7px 10px', fontSize: 'var(--t-s)' }} />
            <input className="inp" name="description" defaultValue={s.description} aria-label="Description" style={{ padding: '7px 10px', fontSize: 'var(--t-lbl)' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ color: 'var(--ink-3)', fontSize: 'var(--t-lbl)' }}>$</span>
              <input className="inp num" name="price" type="number" min={0} step={5}
                defaultValue={s.priceCents / 100} aria-label="Price in dollars" style={{ padding: '7px 10px', fontSize: 'var(--t-s)' }} />
            </div>
            <input className="inp num" name="capacity" type="number" min={0}
              defaultValue={s.capacity} aria-label="Capacity" style={{ padding: '7px 10px', fontSize: 'var(--t-s)' }} />
